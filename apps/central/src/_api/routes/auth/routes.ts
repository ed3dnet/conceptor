...
import { TENANT_USER_AUTH, tH, uH } from "../../http/security.js";
...
async function authRoutes(fastify: AppFastify) {
  fastify.get<{
    ...
    (request, reply) =>
      tH(request, reply, async (tenant, tenantDeps) => {
        const { authConnectors } = tenantDeps;
        const connectors = await authConnectors.list();

        const publicConnectors = await Promise.all(
          connectors.map((c) => authConnectors.toPublic(c)),
        );
  ...
  fastify.get(
    ...
    (request, reply) =>
      tH(request, reply, async (_tenant, tenantDeps) => {
        const { auth, config } = tenantDeps;

        const { originalUrl } = request;
        const { authConnectorId } = request.params;
        const { state } = request.query;

        const sessionRet = await auth.TX_handleOIDCCallback(
          authConnectorId,
          state,
          new URL(config.urls.apiBaseUrl + originalUrl),
        );
  ...
  fastify.get<{
    ...
    (request, reply) =>
      uH(request, reply, async (user, _tenant, tenantDeps) => {
        return tenantDeps.users.getUserPrivate(UserIds.toRichId(user.userId));
      }),
  );
}
