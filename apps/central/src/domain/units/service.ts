...
export class UnitService {
  ...
  async getChildUnits(
    ...
    return executor
      .select()
      .from(UNITS)
      .where(eq(UNITS.parentUnitId, parentUnitId));
  }
  ...
  async getUnitAssignments(
    ...
    const whereClause = includeInactive
      ? eq(UNIT_ASSIGNMENTS.unitId, unitId)
      : and(
          eq(UNIT_ASSIGNMENTS.unitId, unitId),
          isNull(UNIT_ASSIGNMENTS.endDate),
        );
  ...
  async updateUnit(
    ...
    if (
      ...
      updateValues.parentUnitId = input.parentUnitId;
  ...
  async deleteUnit(
    ...
    // Delete the unit
    await executor.delete(UNITS).where(eq(UNITS.id, unitId));
  ...
  async assignUserToUnit(
    ...
    const existingAssignments = await executor
      ...
      .where(
        and(
          eq(UNIT_ASSIGNMENTS.unitId, unitId),
          eq(UNIT_ASSIGNMENTS.userId, input.userId),
          isNull(UNIT_ASSIGNMENTS.endDate),
        ),
      );
  ...
  async unassignUserFromUnit(
    ...
    const [assignment] = await executor
      ...
      .where(
        and(
          eq(UNIT_ASSIGNMENTS.unitId, unitId),
          eq(UNIT_ASSIGNMENTS.userId, userId),
          isNull(UNIT_ASSIGNMENTS.endDate),
        ),
      );
  ...
  async getUnitTags(
    ...
    const tags = await executor
      .select()
      .from(UNIT_TAGS)
      .where(eq(UNIT_TAGS.unitId, unitId));
