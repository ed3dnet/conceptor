...
import { UnitIds, type UnitId } from "../../../domain/units/id.js";
import { UserIds, type UserId } from "../../../domain/users/id.js";
...
export const seed: SeedFn = async (deps, logger) => {
  ...
  try {
    ...
    const tenantId = TenantIds.toRichId("00000000-0000-0000-0000-000000000000");
    const connectorId = AuthConnectorIds.toRichId(
    ...
    await deps.db.transaction(async (tx) => {
      ...
      const userIdMap = new Map<string, UserId>();
      const unitIdMap = new Map<string, UnitId>();
      ...
      for (const user of users) {
        ...
        // Store the mapping
        userIdMap.set(user.id, UserIds.toRichId(dbUser.userId));
      ...
      for (const unit of sortedUnits) {
        ...
        const createdUnit = await tenantDeps.units.createUnit(
          ...
          { squelchEvents: false }, // Squelch events during seeding
          tx,
        );
        ...
        if (unit.kind === "Individual" && unit.employee_id) {
          ...
          if (userId) {
            ...
            await tenantDeps.units.assignUserToUnit(
              ...
              // TODO: later, once we are seeding asks/answers, change this
              //       for now, we don't have any asks/answers in our data set,
              //       so we can let this fire and put all unit assignments
              //       through induction. eventually we will pre-load asks/answers
              //       in our seeding process so we'll want to disable this then.
              { squelchEvents: false },
              tx,
            );
          } else {
