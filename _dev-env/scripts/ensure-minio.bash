...
declare -a BUCKETS=(
    "${CENTRAL_S3__BUCKETS__CORE}"
    "${CENTRAL_S3__BUCKETS__USER_PUBLIC_CONTENT}"
    "${CENTRAL_S3__BUCKETS__USER_SIGNED_ACCESS}"
    "${CENTRAL_S3__BUCKETS__UPLOAD_STAGING}"
)
...
if [ "$CENTRAL_S3__SSL" = "true" ]; then
    PROTOCOL="https://"
else
    PROTOCOL="http://"
fi
ENDPOINT_URL="${PROTOCOL}${CENTRAL_S3__ENDPOINT}"

# Validate endpoint URL
if [ -z "$CENTRAL_S3__ENDPOINT" ]; then
...
mc alias set local "$ENDPOINT_URL" "$CENTRAL_S3__ACCESS_KEY" "$CENTRAL_S3__SECRET_KEY" --api S3v4
    ...
    if mc ls local > /dev/null 2>&1; then
        ...
        for BUCKET in "${BUCKETS[@]}"; do
            ...
            if mc ls "local/${BUCKET}" > /dev/null 2>&1; then
                ...
                if ! mc mb "local/${BUCKET}" > /dev/null 2>&1; then
